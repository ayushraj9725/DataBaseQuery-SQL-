...
CREATE TABLE worker( 
 ...
 -- using GROUP BY and HAVING  for filter out the data by appling aggregiation 
 
 SELECT DEPARTMENT,COUNT(department) FROM worker GROUP BY DEPARTMENT HAVING COUNT(department) > 1 ; 
 
 SELECT SALARY,AVG(salary) FROM worker GROUP BY SALARY HAVING AVG(salary) > 400000 ;
 
 SELECT DEPARTMENT,AVG(salary) FROM worker GROUP BY DEPARTMENT HAVING AVG(salary) > 400000 ;
